...
function eventEventModal(event) { 
    ...
    let eventImage = event.performers && event.performers[0] && event.performers[0].image ? `<img src="${event.performers[0].image}" alt="Event Image" style="max-width: 100%;">` : '<p>No image available</p>';
    let venueDetails = event.venue ? `<p><strong>Location:</strong> ${event.venue.name}, ${event.venue.address}, ${event.venue.city}, ${event.venue.state}</p>` : '<p>Venue details not available</p>';
    let html = `
